...
import { makeBreadcrumbJsonLd } from '@/lib/structured-data'
...
function SiteBreadcrumbs({ currentPath, items }: SiteBreadcrumbsProps) {
  const currentItem = items.at(-1)
  const breadcrumbJsonLd = makeBreadcrumbJsonLd([
    { name: 'Home', path: '/' },
    ...items
      .slice(0, -1)
      .flatMap((item) => (item.href ? [{ name: item.label, path: item.href }] : [])),
    { name: currentItem?.label ?? 'Current page', path: currentPath },
  ])

...
import {
  fnskuLabelMistakesArticle,
  makeFnskuLabelMistakesArticleJsonLd,
} from '@/lib/fnsku-label-mistakes-article'
...
export function FnskuLabelMistakesArticlePage() {
  ...
  return (
    <main className="hs-page min-h-screen overflow-x-hidden antialiased">
      ...
      <SiteBreadcrumbs
        currentPath={fnskuLabelMistakesArticle.href}
        items={[
          { label: 'China sourcing risk guides', href: '/china-sourcing-risk-guides' },
          { label: fnskuLabelMistakesArticle.title },
        ]}
      />
      ...
      <section className="hs-hero">
        <div className="hs-container grid gap-10 py-10 sm:py-14 lg:grid-cols-12 lg:py-16">
          <Reveal className="min-w-0 lg:col-span-6">
            <p className="hs-eyebrow">{fnskuLabelMistakesArticle.eyebrow}</p>
            <h1 className="mt-4 max-w-4xl text-balance text-4xl font-extrabold text-[var(--hs-text)] sm:text-5xl">
              {fnskuLabelMistakesArticle.h1}
            </h1>
            <p className="hs-muted mt-5 max-w-3xl text-base leading-7 sm:text-lg sm:leading-8">
              {fnskuLabelMistakesArticle.intro}
            </p>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <ContactAgentButton
                ...
                analyticsLocation="fnsku_label_mistakes_article_hero"
                className="hs-btn-primary h-12 px-6 text-sm sm:px-8"
                href={whatsappHref}
                size="lg"
                variant="default"
              >
                {fnskuLabelMistakesArticle.primaryCta.label}
              </ContactAgentButton>
              <Button
                asChild
                ...
                variant="outline"
              >
                <a href={fnskuLabelMistakesArticle.secondaryCta.href}>
                  {fnskuLabelMistakesArticle.secondaryCta.label}
                </a>
              </Button>
            </div>
          </Reveal>
            ...
            <div className="overflow-hidden rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-[var(--hs-bg-soft)] shadow-[var(--hs-shadow-md)]">
              <div className="relative aspect-video">
                <Image
                  ...
                  src={fnskuLabelMistakesArticle.image.src}
                />
              </div>
            </div>
          ...
          <aside className="lg:col-span-4">
            <div className="hs-card sticky top-24 bg-[var(--hs-bg-soft)] p-5">
              ...
              <nav aria-label="Article table of contents" className="mt-5 grid gap-2">
                {fnskuLabelMistakesArticle.tableOfContents.map((item) => (
                  <a
          ...
          <article className="min-w-0 lg:col-span-8">
            <Reveal>
              ...
              <p className="hs-muted mt-4 text-base leading-7">
                Check SKU match, barcode readability, label placement, old barcode coverage,
                SKU separation, carton evidence, and correction proof before goods leave China.
              </p>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {fnskuLabelMistakesArticle.quickChecks.map((item, index) => (
                  <div
            ...
            <div className="mt-12 grid gap-12">
              {fnskuLabelMistakesArticle.sections.map((section, index) => (
      ...
      <section className="hs-section-soft" id="decision-table">
        <div className="hs-container hs-section">
          <Reveal className="max-w-3xl">
            <p className="hs-eyebrow">FNSKU buyer decision table</p>
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              What was checked and what the buyer can decide.
            </h2>
            <p className="hs-muted mt-3 text-base leading-7">
              The useful result is not a yes or no label answer. The useful result is
              a shipping decision before cartons leave China.
            </p>
          </Reveal>
          <div className="mt-8 overflow-hidden rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-white shadow-[var(--hs-shadow-sm)]">
            ...
            {fnskuLabelMistakesArticle.decisionRows.map((row) => (
              <div
      ...
      <section className="hs-section-white" id="documents">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <p className="hs-muted mt-3 text-base leading-7">
              Send final label files and SKU details before pickup so the check can
              focus on the real label mistake risk, not generic FBA prep advice.
            </p>
          </Reveal>
          <Reveal className="lg:col-span-8">
            <BulletList items={fnskuLabelMistakesArticle.whatToSend} />
          </Reveal>
        </div>
      </section>
      ...
      <section className="hs-section-soft" id="red-flags">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <p className="hs-muted mt-3 text-base leading-7">
              These signals mean the buyer should slow down the pickup decision and ask
              for stronger SKU-level or carton-level evidence.
            </p>
          </Reveal>
          <Reveal className="lg:col-span-8">
            <BulletList items={fnskuLabelMistakesArticle.redFlags} />
          </Reveal>
        </div>
      </section>
      ...
      <section className="hs-section-white" id="limits">
        <div className="hs-container hs-section max-w-4xl">
          <Reveal>
            ...
            <p className="hs-muted mt-3 text-base leading-7">
              A label check reduces visible China-side label risk. It does not make every
              Amazon, compliance, or receiving issue disappear after shipment.
            </p>
            <BulletList items={fnskuLabelMistakesArticle.scopeLimits} />
          </Reveal>
        </div>
      </section>
      ...
      <section className="hs-section-soft">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              Move from label risk to the right FBA check.
            </h2>
          </Reveal>
          <div className="grid gap-3 lg:col-span-8 md:grid-cols-2">
            {fnskuLabelMistakesArticle.relatedLinks.map((link, index) => (
              <Link
      ...
      <section className="hs-section-white" id="faq">
        <div className="hs-container hs-section max-w-4xl">
          ...
          <div className="mt-6 grid gap-3">
            {fnskuLabelMistakesArticle.faqs.map((faq, index) => (
              <Reveal
                as="article"
        ...
        <div className="hs-container hs-section">
          <Reveal className="hs-cta-band px-6 py-10 text-white sm:px-10 lg:flex lg:items-center lg:justify-between lg:gap-10">
            <div className="relative">
              <p className="text-sm font-extrabold text-red-200">Before goods leave China</p>
              <h2 className="mt-3 text-3xl font-extrabold">
                Ask for FBA prep before an FNSKU mistake becomes overseas rework.
              </h2>
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-300">
                Send FNSKU files, SKU list, carton count, pickup date, and the label
                mistake you are worried about.
              </p>
            </div>
            <ContactAgentButton
              analyticsLabel={fnskuLabelMistakesArticle.primaryCta.label}
              analyticsLocation="fnsku_label_mistakes_article_final"

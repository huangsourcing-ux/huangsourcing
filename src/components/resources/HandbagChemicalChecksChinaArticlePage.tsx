...
import { handbagChemicalChecksChinaArticle, makeHandbagChemicalChecksChinaArticleJsonLd } from '@/lib/handbag-chemical-checks-china-article'
...
export function HandbagChemicalChecksChinaArticlePage() {
  ...
  return (
    <main className="hs-page min-h-screen overflow-x-clip antialiased">
      ...
      <JsonLd data={makeHandbagChemicalChecksChinaArticleJsonLd()} />
        ...
        <div className="hs-container grid gap-10 py-10 sm:py-14 lg:grid-cols-12 lg:py-16">
          <Reveal className="min-w-0 lg:col-span-6">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <ContactAgentButton analyticsLabel={article.primaryCta.label} analyticsLocation="handbag_chemical_article_hero" className="hs-btn-primary min-h-12 px-6 text-sm" href={whatsappHref} size="lg" variant="default">{article.primaryCta.label}</ContactAgentButton>
          ...
          <Reveal className="min-w-0 self-center lg:col-span-6">
            <figure className="overflow-hidden rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-white shadow-[var(--hs-shadow-sm)]">
              ...
              <figcaption className="hs-muted px-5 py-4 text-sm leading-6">Original buyer-side workflow diagram. It illustrates evidence handoffs; it is not a laboratory result or a photograph of a recalled product.</figcaption>
            </figure>
          </Reveal>
        </div>
      </section>
      <section className="hs-section-white scroll-mt-24" id="quick-answer">
        <div className="hs-container hs-section grid gap-10 lg:grid-cols-12">
          ...
          <article className="min-w-0 lg:col-span-8">
            <Reveal><p className="hs-eyebrow">Quick answer</p><h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">What must match before the handbags leave China?</h2>
            ...
            <Reveal as="section" className="mt-12 scroll-mt-24" id="release-checklist"><p className="hs-eyebrow">China-side evidence module</p><h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">Handbag chemical evidence release checklist</h2>
      ...
      <EvidenceDecisionMatrix id="decision-table" intro="Name the style, colorway, component evidence, production lot, and carton range in every disposition. Keep technical clearance, correction evidence, and commercial release visible." rows={article.decisionRows} title="Turn the evidence into a release decision" />
      <EvidenceBasisSection className="hs-section-white" intro="Researched from cited public records and official chemical guidance, with an original buyer-side inspection and release workflow." items={article.evidenceBasis} />
      <section className="hs-section-soft scroll-mt-24" id="public-case"><div className="hs-container hs-section max-w-4xl"><Reveal>
        ...
        <div className="hs-muted mt-4 grid gap-4 text-base leading-7">
          ...
          <p>{article.secondCase} <a className="hs-text-link" href={article.secondCaseHref} rel="noreferrer" target="_blank">RappelConso Kiabi COH67 recall</a>.</p>
        </div>
      </Reveal></div></section>
      ...
      <section className="bg-[var(--hs-navy)] text-white"><div className="hs-container hs-section"><Reveal className="grid items-center gap-8 lg:grid-cols-12"><div className="lg:col-span-8"><ShieldCheck aria-hidden className="size-7 text-[var(--hs-accent)]" /><h2 className="mt-4 text-3xl font-extrabold">Make the handbag release scope traceable.</h2><p className="mt-4 max-w-3xl text-base leading-7 text-white/75">Send the component map, approved references, complete evidence, production lots, carton map, and deadline. Huang Sourcing can scope a practical China-side comparison before balance payment or pickup.</p></div><div className="flex flex-col gap-3 lg:col-span-4">
        <ContactAgentButton analyticsLabel="Check handbags before shipment" analyticsLocation="handbag_chemical_article_final_cta" className="hs-btn-primary min-h-12 px-6" href={whatsappHref} size="lg" variant="default">Check Before Shipment</ContactAgentButton>

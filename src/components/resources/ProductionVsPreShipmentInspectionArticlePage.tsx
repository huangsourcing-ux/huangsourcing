...
import {
  makeProductionVsPreShipmentInspectionArticleJsonLd,
  productionVsPreShipmentInspectionArticle,
} from '@/lib/production-vs-pre-shipment-inspection-article'
...
export function ProductionVsPreShipmentInspectionArticlePage() {
  const whatsappHref = buildWhatsAppHref(
    productionVsPreShipmentInspectionArticle.whatsappMessage,
  )

  return (
    <main className="hs-page min-h-screen overflow-x-clip antialiased">
      <SiteHeader activePage="resources" topBanner={null} />
      <JsonLd data={makeProductionVsPreShipmentInspectionArticleJsonLd()} />
      <SiteBreadcrumbs
        currentPath={productionVsPreShipmentInspectionArticle.href}
        items={[
          { label: 'China sourcing risk guides', href: '/china-sourcing-risk-guides' },
          { label: productionVsPreShipmentInspectionArticle.title },
        ]}
      />
      ...
      <section className="hs-hero">
        <div className="hs-container grid gap-10 py-10 sm:py-14 lg:grid-cols-12 lg:py-16">
          <Reveal className="min-w-0 lg:col-span-6">
            <p className="hs-eyebrow">{productionVsPreShipmentInspectionArticle.eyebrow}</p>
            <h1 className="mt-4 max-w-4xl text-balance text-4xl font-extrabold text-[var(--hs-text)] sm:text-5xl">
              {productionVsPreShipmentInspectionArticle.h1}
            </h1>
            <ArticleByline
              publishedDate={productionVsPreShipmentInspectionArticle.publishedDate}
            />
            <p className="hs-muted mt-5 max-w-3xl text-base leading-7 sm:text-lg sm:leading-8">
              {productionVsPreShipmentInspectionArticle.intro}
            </p>
            <p className="hs-muted mt-4 max-w-3xl text-base leading-7">
              {productionVsPreShipmentInspectionArticle.answerSummary}
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <ContactAgentButton
                analyticsLabel={productionVsPreShipmentInspectionArticle.primaryCta.label}
                analyticsLocation="production_vs_pre_shipment_article_hero"
                className="hs-btn-primary h-12 px-6 text-sm sm:px-8"
                href={whatsappHref}
                size="lg"
                variant="default"
              >
                {productionVsPreShipmentInspectionArticle.primaryCta.label}
              </ContactAgentButton>
              <Button
                asChild
                ...
                variant="outline"
              >
                <a href={productionVsPreShipmentInspectionArticle.secondaryCta.href}>
                  {productionVsPreShipmentInspectionArticle.secondaryCta.label}
                </a>
              </Button>
            </div>
          </Reveal>
          ...
          <Reveal className="lg:col-span-6" delayMs={120}>
            <div className="overflow-hidden rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-[var(--hs-bg-soft)] shadow-[var(--hs-shadow-md)]">
              <div className="relative aspect-video">
                <Image
                  alt={productionVsPreShipmentInspectionArticle.image.alt}
                  className="object-cover"
                  fill
                  priority
                  sizes="(min-width: 1024px) 50vw, 100vw"
                  src={productionVsPreShipmentInspectionArticle.image.src}
                />
              </div>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {['DUPRO', 'PSI', 'Rework', 'Release'].map((item) => (
                <a
          ...
          <aside className="lg:col-span-4">
            <div className="hs-card sticky top-24 bg-[var(--hs-bg-soft)] p-5">
              ...
              <nav aria-label="Article table of contents" className="mt-5 grid gap-2">
                {productionVsPreShipmentInspectionArticle.tableOfContents.map((item) => (
                  <a
          ...
          <article className="min-w-0 lg:col-span-8">
            <Reveal>
              ...
              <p className="hs-muted mt-4 text-base leading-7">
                Inspect during production when early correction can still change the
                order. Inspect before shipment when finished goods, packing, labels,
                cartons, balance payment, pickup, or shipment release need final
                buyer-side evidence.
              </p>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {productionVsPreShipmentInspectionArticle.quickChecks.map((item, index) => (
                  <div
            ...
            <div className="mt-12 grid gap-12">
              {productionVsPreShipmentInspectionArticle.sections.map((section, index) => (
                <Reveal
                  as="section"
      ...
      <section className="hs-section-soft" id="decision-table">
        <div className="hs-container hs-section">
          <Reveal className="max-w-3xl">
            <p className="hs-eyebrow">Inspection timing table</p>
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              Match inspection timing to the buyer decision.
            </h2>
            <p className="hs-muted mt-3 text-base leading-7">
              The useful inspection is the one that answers the next real decision:
              correct production, hold payment, request rework, delay pickup, release
              shipment, or re-check.
            </p>
          </Reveal>
          <div className="mt-8 overflow-hidden rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-white shadow-[var(--hs-shadow-sm)]">
            ...
            {productionVsPreShipmentInspectionArticle.decisionRows.map((row) => (
              <div
      ...
      <EvidenceBasisSection
        intro="This timing guidance is based on production status, product and spec evidence, packing and label evidence, buyer payment or pickup timing, and what can still be corrected in China."
        items={productionVsPreShipmentInspectionArticle.evidenceBasis}
      />
        ...
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <h2 className="mt-5 text-3xl font-extrabold text-[var(--hs-text)]">
              What to send before choosing timing.
            </h2>
            <p className="hs-muted mt-3 text-base leading-7">
              Send the production stage, product evidence, packing details, and next
              payment or shipment deadline so the inspection timing matches the risk.
            </p>
          </Reveal>
          <Reveal className="lg:col-span-8">
            <BulletList items={productionVsPreShipmentInspectionArticle.whatToSend} />
          </Reveal>
        </div>
      </section>
      ...
      <section className="hs-section-soft" id="red-flags">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <h2 className="mt-5 text-3xl font-extrabold text-[var(--hs-text)]">
              Red flags that timing is becoming risky.
            </h2>
            <p className="hs-muted mt-3 text-base leading-7">
              If these signals appear, choose an inspection stage before supplier
              pressure removes correction, payment, or pickup leverage.
            </p>
          </Reveal>
          <Reveal className="lg:col-span-8">
            <BulletList items={productionVsPreShipmentInspectionArticle.redFlags} />
          </Reveal>
        </div>
      </section>
      ...
      <section className="hs-section-white" id="limits">
        <div className="hs-container hs-section max-w-4xl">
          <Reveal>
            ...
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              What inspection timing cannot guarantee.
            </h2>
            <p className="hs-muted mt-3 text-base leading-7">
              Timing helps match evidence to leverage, but no onsite inspection can
              replace clear specifications, lab testing, certification review, legal
              advice, customs advice, or marketplace compliance checks.
            </p>
            <BulletList items={productionVsPreShipmentInspectionArticle.scopeLimits} />
          </Reveal>
        </div>
      </section>
      ...
      <section className="hs-section-soft">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              Move from timing question to the right check.
            </h2>
          </Reveal>
          <div className="grid gap-3 lg:col-span-8 md:grid-cols-2">
            {productionVsPreShipmentInspectionArticle.relatedLinks.map((link, index) => (
              <Link
      ...
      <section className="hs-section-white" id="faq">
        <div className="hs-container hs-section max-w-4xl">
          ...
          <div className="mt-6 grid gap-3">
            {productionVsPreShipmentInspectionArticle.faqs.map((faq, index) => (
              <Reveal
                as="article"
        ...
        <div className="hs-container hs-section">
          <Reveal className="hs-cta-band px-6 py-10 text-white sm:px-10 lg:flex lg:items-center lg:justify-between lg:gap-10">
            <div className="relative">
              <p className="text-sm font-extrabold text-red-200">Choose inspection timing</p>
              <h2 className="mt-3 text-3xl font-extrabold">
                Decide before payment, pickup, or production momentum takes over.
              </h2>
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-300">
                Send the product, factory location, production stage, order quantity,
                payment or pickup deadline, and main concern. Agent Huang will help
                scope whether a DUPRO, PSI, or both checks fit the decision.
              </p>
            </div>
            <ContactAgentButton
              analyticsLabel={productionVsPreShipmentInspectionArticle.primaryCta.label}
              analyticsLocation="production_vs_pre_shipment_article_final"

...
import {
  chineseSupplierDepositDecisionArticle,
  makeChineseSupplierDepositDecisionArticleJsonLd,
} from '@/lib/chinese-supplier-deposit-decision-article'
...
export function ChineseSupplierDepositDecisionArticlePage() {
  const whatsappHref = buildWhatsAppHref(chineseSupplierDepositDecisionArticle.whatsappMessage)

  return (
    <main className="hs-page min-h-screen overflow-x-clip antialiased">
      <SiteHeader activePage="resources" topBanner={null} />
      <JsonLd data={makeChineseSupplierDepositDecisionArticleJsonLd()} />
      <SiteBreadcrumbs
        currentPath={chineseSupplierDepositDecisionArticle.href}
        items={[
          { label: 'China sourcing risk guides', href: '/china-sourcing-risk-guides' },
          { label: chineseSupplierDepositDecisionArticle.title },
        ]}
      />
      ...
      <section className="hs-hero">
        <div className="hs-container grid gap-10 py-10 sm:py-14 lg:grid-cols-12 lg:py-16">
          <Reveal className="min-w-0 lg:col-span-6">
            <p className="hs-eyebrow">{chineseSupplierDepositDecisionArticle.eyebrow}</p>
            <h1 className="mt-4 max-w-4xl text-balance text-4xl font-extrabold text-[var(--hs-text)] sm:text-5xl">
              {chineseSupplierDepositDecisionArticle.h1}
            </h1>
            <p className="hs-muted mt-5 max-w-3xl text-base leading-7 sm:text-lg sm:leading-8">
              {chineseSupplierDepositDecisionArticle.intro}
            </p>
            <p className="hs-muted mt-4 max-w-3xl text-base leading-7">
              {chineseSupplierDepositDecisionArticle.answerSummary}
            </p>
            <ArticleByline
              modifiedDate={chineseSupplierDepositDecisionArticle.modifiedDate}
              publishedDate={chineseSupplierDepositDecisionArticle.publishedDate}
            />
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <ContactAgentButton
                analyticsLabel={chineseSupplierDepositDecisionArticle.primaryCta.label}
                analyticsLocation="chinese_supplier_deposit_decision_article_hero"
                className="hs-btn-primary h-12 px-6 text-sm sm:px-8"
                href={whatsappHref}
                size="lg"
                variant="default"
              >
                {chineseSupplierDepositDecisionArticle.primaryCta.label}
              </ContactAgentButton>
              <Button
                asChild
                ...
                variant="outline"
              >
                <a href={chineseSupplierDepositDecisionArticle.secondaryCta.href}>
                  {chineseSupplierDepositDecisionArticle.secondaryCta.label}
                </a>
              </Button>
            </div>
          </Reveal>
            ...
            <div className="overflow-hidden rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-[var(--hs-bg-soft)] shadow-[var(--hs-shadow-md)]">
              <div className="relative aspect-video">
                <Image
                  ...
                  src={chineseSupplierDepositDecisionArticle.image.src}
                />
              </div>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {['Identity', 'Bank', 'PI', 'Checkpoint'].map((item) => (
                <a
          ...
          <aside className="lg:col-span-4">
            <div className="hs-card sticky top-24 bg-[var(--hs-bg-soft)] p-5">
              ...
              <nav aria-label="Article table of contents" className="mt-5 grid gap-2">
                {chineseSupplierDepositDecisionArticle.tableOfContents.map((item) => (
                  <a
          ...
          <article className="min-w-0 lg:col-span-8">
            <Reveal>
              ...
              <p className="hs-muted mt-4 text-base leading-7">
                A supplier deposit should be released only after the company identity,
                bank beneficiary, PI, product scope, and next inspection checkpoint
                are clear enough for the buyer decision. If those pieces do not line
                up, pause and ask for proof before payment.
              </p>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {chineseSupplierDepositDecisionArticle.quickChecks.map((item, index) => (
                  <div
            ...
            <div className="mt-12 grid gap-12">
              {chineseSupplierDepositDecisionArticle.sections.map((section, index) => (
        ...
        <div className="hs-container hs-section">
          <Reveal className="max-w-3xl">
            ...
            <p className="hs-muted mt-3 text-base leading-7">
              The deposit decision should be tied to visible evidence. Use the check
              results to decide whether to pay, hold, request proof, revise terms, or stop.
            </p>
          </Reveal>
          <div className="mt-8 overflow-hidden rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-white shadow-[var(--hs-shadow-sm)]">
            ...
            {chineseSupplierDepositDecisionArticle.decisionRows.map((row) => (
              <div
      ...
      <EvidenceBasisSection
        intro="This deposit guidance is based on the supplier identity, payment path, PI scope, product evidence, and buyer-stage facts available before money moves."
        items={chineseSupplierDepositDecisionArticle.evidenceBasis}
      />
        ...
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <h2 className="mt-5 text-3xl font-extrabold text-[var(--hs-text)]">
              What to send before the deposit check.
            </h2>
            <p className="hs-muted mt-3 text-base leading-7">
              Strong inputs help separate normal sourcing friction from a real deposit risk.
              Send the supplier evidence before money moves.
            </p>
          </Reveal>
          <Reveal className="lg:col-span-8">
            <BulletList items={chineseSupplierDepositDecisionArticle.whatToSend} />
          </Reveal>
        </div>
      </section>
      ...
      <section className="hs-section-soft" id="red-flags">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <h2 className="mt-5 text-3xl font-extrabold text-[var(--hs-text)]">
              Red flags before paying a 30% deposit.
            </h2>
            <p className="hs-muted mt-3 text-base leading-7">
              One warning sign may have a normal explanation. Several together usually
              mean the buyer should slow down before transfer.
            </p>
          </Reveal>
          <Reveal className="lg:col-span-8">
            <BulletList items={chineseSupplierDepositDecisionArticle.redFlags} />
          </Reveal>
        </div>
      </section>
      ...
      <section className="hs-section-white" id="limits">
        <div className="hs-container hs-section max-w-4xl">
          <Reveal>
            ...
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              What a before-deposit check cannot guarantee.
            </h2>
            <p className="hs-muted mt-3 text-base leading-7">
              Honest limits keep the decision useful. Supplier verification can reduce
              visible risk before deposit, but it should not be treated as unlimited certainty.
            </p>
            <BulletList items={chineseSupplierDepositDecisionArticle.scopeLimits} />
          </Reveal>
        </div>
      </section>
      ...
      <section className="hs-section-soft">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              Move from deposit doubt to the right China-side check.
            </h2>
          </Reveal>
          <div className="grid gap-3 lg:col-span-8 md:grid-cols-2">
            {chineseSupplierDepositDecisionArticle.relatedLinks.map((link, index) => (
              <Link
      ...
      <section className="hs-section-white" id="faq">
        <div className="hs-container hs-section max-w-4xl">
          ...
          <div className="mt-6 grid gap-3">
            {chineseSupplierDepositDecisionArticle.faqs.map((faq, index) => (
              <Reveal
                as="article"
          ...
          <Reveal className="hs-cta-band px-6 py-10 text-white sm:px-10 lg:flex lg:items-center lg:justify-between lg:gap-10">
            <div className="relative">
              ...
              <h2 className="mt-3 text-3xl font-extrabold">
                Send the supplier details before you release the 30% deposit.
              </h2>
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-300">
                Share the supplier link, PI, bank details, product scope, and payment
                deadline. Agent Huang will help review the visible deposit risk from China.
              </p>
            </div>
            <ContactAgentButton
              analyticsLabel={chineseSupplierDepositDecisionArticle.primaryCta.label}
              analyticsLocation="chinese_supplier_deposit_decision_article_final"

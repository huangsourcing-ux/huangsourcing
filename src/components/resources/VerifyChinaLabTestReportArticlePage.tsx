import {
  AlertTriangle,
  ArrowRight,
  CheckCircle2,
  ClipboardCheck,
  ExternalLink,
  FileCheck2,
  FileSearch,
  FlaskConical,
  Scale,
  ShieldCheck,
} from 'lucide-react'
import Image from 'next/image'
import Link from 'next/link'

import { ContactAgentButton } from '@/components/home/ContactAgentButton'
import {
  ArticleByline,
  EvidenceBasisSection,
} from '@/components/resources/ArticleTrustSignals'
import { JsonLd } from '@/components/seo/JsonLd'
import { Reveal } from '@/components/site/Reveal'
import { SiteBreadcrumbs } from '@/components/site/SiteBreadcrumbs'
import { SiteFooter } from '@/components/site/SiteFooter'
import { SiteHeader } from '@/components/site/SiteHeader'
import { Button } from '@/components/ui/button'
import { buildWhatsAppHref } from '@/lib/site-links'
import {
  makeVerifyChinaLabTestReportArticleJsonLd,
  verifyChinaLabTestReportArticle,
} from '@/lib/verify-china-lab-test-report-article'

function BulletList({ items }: { items: readonly string[] }) {
  return (
    <ul className="mt-4 grid gap-3 text-sm leading-6 text-[var(--hs-muted)]">
      {items.map((item) => (
        <li className="flex gap-2" key={item}>
          <CheckCircle2
            aria-hidden
            className="mt-0.5 size-4 shrink-0 text-[var(--hs-accent)]"
          />
          <span>{item}</span>
        </li>
      ))}
    </ul>
  )
}

export function VerifyChinaLabTestReportArticlePage() {
  const article = verifyChinaLabTestReportArticle
  const whatsappHref = buildWhatsAppHref(article.whatsappMessage)

  return (
    <main className="hs-page min-h-screen overflow-x-clip antialiased">
      <SiteHeader activePage="resources" topBanner={null} />
      <JsonLd data={makeVerifyChinaLabTestReportArticleJsonLd()} />
      <SiteBreadcrumbs
        currentPath={article.href}
        items={[
          { label: 'China sourcing risk guides', href: '/china-sourcing-risk-guides' },
          { label: article.title },
        ]}
      />

      <section className="hs-hero">
        <div className="hs-container grid gap-10 py-10 sm:py-14 lg:grid-cols-12 lg:py-16">
          <Reveal className="min-w-0 lg:col-span-6">
            <p className="hs-eyebrow">{article.eyebrow}</p>
            <h1 className="mt-4 max-w-4xl text-balance text-4xl font-extrabold text-[var(--hs-text)] sm:text-5xl">
              {article.h1}
            </h1>
            <ArticleByline
              author={article.author}
              modifiedDate={article.modifiedDate}
              publishedDate={article.publishedDate}
            />
            <p className="hs-muted mt-5 max-w-3xl text-base leading-7 sm:text-lg sm:leading-8">
              {article.intro}
            </p>
            <p className="hs-muted mt-4 max-w-3xl text-base leading-7">
              {article.answerSummary}
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <ContactAgentButton
                analyticsLabel={article.primaryCta.label}
                analyticsLocation="verify_china_lab_report_article_hero"
                className="hs-btn-primary h-12 px-6 text-sm sm:px-8"
                href={whatsappHref}
                size="lg"
                variant="default"
              >
                {article.primaryCta.label}
              </ContactAgentButton>
              <Button
                asChild
                className="hs-btn-secondary h-12 px-6 text-sm sm:px-8"
                size="lg"
                variant="outline"
              >
                <a href={article.secondaryCta.href}>{article.secondaryCta.label}</a>
              </Button>
            </div>
          </Reveal>

          <Reveal className="lg:col-span-6" delayMs={120}>
            <div className="overflow-hidden rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-[var(--hs-bg-soft)] shadow-[var(--hs-shadow-md)]">
              <div className="relative aspect-video">
                <Image
                  alt={article.image.alt}
                  className="object-cover"
                  fill
                  priority
                  sizes="(min-width: 1024px) 50vw, 100vw"
                  src={article.image.src}
                  unoptimized
                />
              </div>
            </div>
            <p className="hs-muted mt-3 text-xs leading-5">
              Neutral illustrative evidence-review scene. It is not a laboratory
              report, a compliance finding, or evidence from the public case below.
            </p>
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {['Lab status', 'Rule scope', 'Product match', 'Release'].map((item) => (
                <a
                  className="min-h-24 rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-white p-3 text-sm font-extrabold text-[var(--hs-text)] shadow-[var(--hs-shadow-sm)] transition-colors hover:border-[var(--hs-accent)] hover:text-[var(--hs-accent-strong)]"
                  href="#report-checklist"
                  key={item}
                >
                  <span className="block text-xs uppercase text-[var(--hs-accent)]">
                    Verify
                  </span>
                  <span className="mt-2 block leading-5">{item}</span>
                </a>
              ))}
            </div>
          </Reveal>
        </div>
      </section>

      <section className="hs-section-white" id="quick-answer">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <aside className="lg:col-span-4">
            <div className="hs-card sticky top-24 bg-[var(--hs-bg-soft)] p-5">
              <div className="hs-icon-box size-12">
                <ClipboardCheck aria-hidden className="size-6" />
              </div>
              <h2 className="mt-5 text-2xl font-extrabold text-[var(--hs-text)]">
                Article guide
              </h2>
              <nav aria-label="Article table of contents" className="mt-5 grid gap-2">
                {article.tableOfContents.map((item) => (
                  <a
                    className="inline-flex min-h-10 items-center justify-between gap-3 rounded-md bg-white px-3 text-sm font-extrabold text-[var(--hs-text)] ring-1 ring-[var(--hs-border)] transition-colors hover:text-[var(--hs-accent-strong)] hover:ring-[var(--hs-accent)]"
                    href={item.href}
                    key={item.href}
                  >
                    <span>{item.label}</span>
                    <ArrowRight
                      aria-hidden
                      className="size-4 shrink-0 text-[var(--hs-muted-soft)]"
                    />
                  </a>
                ))}
              </nav>
            </div>
          </aside>

          <article className="min-w-0 lg:col-span-8">
            <Reveal>
              <p className="hs-eyebrow">Quick answer</p>
              <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
                What should a buyer verify before relying on a China lab report?
              </h2>
              <p className="hs-muted mt-4 text-base leading-7">
                Verify the named laboratory in the official directory, match every
                required rule to its accepted scope on the test date, confirm the full
                report directly with the lab when needed, and connect the tested sample
                to the exact factory, product version, materials, labels, lots, and
                cartons being released.
              </p>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {article.quickChecks.map((item, index) => (
                  <div
                    className="flex min-h-16 gap-3 rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-[var(--hs-card-warm)] p-4 text-sm leading-6 text-[var(--hs-muted)]"
                    key={item}
                  >
                    <span className="flex size-7 shrink-0 items-center justify-center rounded-md bg-[var(--hs-navy)] text-xs font-extrabold text-white">
                      {index + 1}
                    </span>
                    <span>{item}</span>
                  </div>
                ))}
              </div>
            </Reveal>

            <Reveal as="section" className="mt-12 scroll-mt-24" id="report-checklist">
              <div className="flex items-center gap-3">
                <div className="hs-icon-box size-12">
                  <FileSearch aria-hidden className="size-6" />
                </div>
                <div>
                  <p className="hs-eyebrow">Buyer evidence module</p>
                  <h2 className="mt-1 text-3xl font-extrabold text-[var(--hs-text)]">
                    Lab report acceptance and release checklist
                  </h2>
                </div>
              </div>
              <p className="hs-muted mt-4 text-base leading-7">
                This checklist organizes evidence for qualified review. It does not
                authenticate a report, decide legal scope, or replace controlled
                laboratory testing.
              </p>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {article.checkGroups.map((group) => (
                  <section
                    className="rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-[var(--hs-bg-soft)] p-5 shadow-[var(--hs-shadow-sm)]"
                    key={group.title}
                  >
                    <div className="flex items-center gap-2">
                      <FileCheck2 aria-hidden className="size-5 text-[var(--hs-accent)]" />
                      <h3 className="text-lg font-extrabold text-[var(--hs-text)]">
                        {group.title}
                      </h3>
                    </div>
                    <BulletList items={group.items} />
                  </section>
                ))}
              </div>
            </Reveal>

            <div className="mt-12 grid gap-12">
              {article.sections.map((section, index) => (
                <Reveal
                  as="section"
                  className="scroll-mt-24"
                  id={section.id}
                  key={section.id}
                  staggerIndex={index}
                >
                  <h2 className="text-3xl font-extrabold text-[var(--hs-text)]">
                    {section.title}
                  </h2>
                  <div className="hs-muted mt-4 grid gap-4 text-base leading-7">
                    {section.paragraphs.map((paragraph) => (
                      <p key={paragraph}>{paragraph}</p>
                    ))}
                  </div>
                  {section.bullets ? <BulletList items={section.bullets} /> : null}
                  {section.id === 'why-current' ? (
                    <p className="hs-muted mt-5 text-sm leading-6">
                      Current official basis:{' '}
                      <a
                        className="hs-text-link"
                        href="https://www.cpsc.gov/About-CPSC/Chairman/Peter-A-Feldman/Statement/CPSC-Immediately-Withdraws-Accreditation-from-Chinese-Laboratories-to-Protect-American-Families"
                        rel="noreferrer"
                        target="_blank"
                      >
                        CPSC four-laboratory withdrawal statement
                      </a>{' '}
                      and the{' '}
                      <a
                        className="hs-text-link"
                        href="https://www.cpsc.gov/Newsroom/News-Releases/2026/US-Consumer-Product-Safety-Commission-Launches-Crackdown-on-Fake-Safety-Labels-Used-to-Push-Dangerous-Foreign-Products-into-US-Homes"
                        rel="noreferrer"
                        target="_blank"
                      >
                        CPSC counterfeit certification mark crackdown
                      </a>
                      .
                    </p>
                  ) : null}
                  {section.id === 'lab-status' ? (
                    <p className="hs-muted mt-5 text-sm leading-6">
                      Verify with the{' '}
                      <a
                        className="hs-text-link"
                        href="https://www.cpsc.gov/cgi-bin/labsearch/"
                        rel="noreferrer"
                        target="_blank"
                      >
                        official CPSC accepted laboratory directory
                      </a>{' '}
                      and separately check the{' '}
                      <a
                        className="hs-text-link"
                        href="https://www.cpsc.gov/cgi-bin/labsearch/ViewInactiveLabs.aspx"
                        rel="noreferrer"
                        target="_blank"
                      >
                        inactive laboratory and adverse-action list
                      </a>
                      .
                    </p>
                  ) : null}
                  {section.id === 'production-change' ? (
                    <p className="hs-muted mt-5 text-sm leading-6">
                      Rule-specific testing, supplier-report reliance with due care, and
                      material-change retesting are addressed in the{' '}
                      <a
                        className="hs-text-link"
                        href="https://www.cpsc.gov/Business--Manufacturing/Testing-Certification/Third-Party-Testing"
                        rel="noreferrer"
                        target="_blank"
                      >
                        CPSC third-party testing guidance
                      </a>
                      .
                    </p>
                  ) : null}
                </Reveal>
              ))}
            </div>
          </article>
        </div>
      </section>

      <section className="hs-section-soft scroll-mt-24" id="public-case">
        <div className="hs-container hs-section">
          <Reveal className="max-w-3xl">
            <p className="hs-eyebrow">Public case example</p>
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              CPSC withdrew acceptance from four China-based laboratories in 2026.
            </h2>
          </Reveal>
          <div className="mt-8 grid gap-5 lg:grid-cols-2">
            <Reveal className="hs-card bg-white p-6">
              <div className="flex items-center gap-3">
                <FlaskConical className="size-6 text-[var(--hs-accent)]" aria-hidden />
                <h3 className="text-xl font-extrabold text-[var(--hs-text)]">
                  What the public record says
                </h3>
              </div>
              <p className="hs-muted mt-4 text-sm leading-7">
                CPSC&apos;s January 15 statement names Shenzhen GTT Testing Technology,
                Dongguan True Safety Testing, Fujian Berton Testing Service, and Shenzhen
                HUAK Testing Technology. The agency described different reasons across
                the four actions, including falsified or unreliable reports, undisclosed
                adverse accreditation status or relocation, later independent test
                failures, and systemic testing-procedure failures.
              </p>
              <p className="hs-muted mt-4 text-sm leading-7">
                The record is an agency action, not a finding that every China-based
                laboratory or every supplier report is unreliable.
              </p>
            </Reveal>
            <Reveal className="hs-card bg-[var(--hs-card-warm)] p-6" delayMs={80}>
              <div className="flex items-center gap-3">
                <Scale className="size-6 text-[var(--hs-accent)]" aria-hidden />
                <h3 className="text-xl font-extrabold text-[var(--hs-text)]">
                  Buyer lesson and limits
                </h3>
              </div>
              <p className="hs-muted mt-4 text-sm leading-7">
                Do not treat a PDF, badge, QR code, or known group name as the complete
                evidence chain. Verify the exact lab entity, location, status, date and
                rule scope, then match the tested sample to production and resolve
                changes before relying on the report.
              </p>
              <p className="hs-muted mt-4 text-sm leading-7">
                This comparison cannot determine fraud, authenticate a document, prove
                safety, or replace the responsible importer&apos;s qualified compliance,
                testing, certificate, and release decisions.
              </p>
              <a
                className="hs-text-link mt-5 inline-flex items-center gap-2 text-sm font-extrabold"
                href="https://www.cpsc.gov/About-CPSC/Chairman/Peter-A-Feldman/Statement/CPSC-Immediately-Withdraws-Accreditation-from-Chinese-Laboratories-to-Protect-American-Families"
                rel="noreferrer"
                target="_blank"
              >
                Read the CPSC primary record
                <ExternalLink aria-hidden className="size-4" />
              </a>
            </Reveal>
          </div>
        </div>
      </section>

      <section className="hs-section-white scroll-mt-24" id="decision-table">
        <div className="hs-container hs-section">
          <Reveal className="max-w-3xl">
            <p className="hs-eyebrow">Release decision table</p>
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              Turn report evidence into an accept, verify, retest, or hold decision.
            </h2>
          </Reveal>
          <div className="mt-8 overflow-x-auto rounded-[var(--hs-radius)] border border-[var(--hs-border)] bg-white shadow-[var(--hs-shadow-sm)]">
            <table className="w-full min-w-3xl border-collapse text-left text-sm leading-6">
              <thead className="bg-[var(--hs-navy)] text-white">
                <tr>
                  <th className="p-4 font-extrabold" scope="col">Risk node</th>
                  <th className="border-l border-white/10 p-4 font-extrabold" scope="col">Evidence to confirm</th>
                  <th className="border-l border-white/10 p-4 font-extrabold" scope="col">Buyer decision</th>
                </tr>
              </thead>
              <tbody>
                {article.decisionRows.map((row) => (
                  <tr className="border-t border-[var(--hs-border)]" key={row.riskNode}>
                    <th className="bg-[var(--hs-bg-soft)] p-4 align-top font-extrabold text-[var(--hs-text)]" scope="row">
                      {row.riskNode}
                    </th>
                    <td className="border-l border-[var(--hs-border)] p-4 align-top text-[var(--hs-muted)]">{row.evidence}</td>
                    <td className="border-l border-[var(--hs-border)] p-4 align-top text-[var(--hs-muted)]">{row.buyerDecision}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      </section>

      <EvidenceBasisSection
        className="hs-section-soft"
        intro="This guide is based on cited 2026 CPSC actions, current official laboratory and testing guidance, an independent GAO oversight review, and buyer-side identity and production evidence that can be compared before final payment or shipment."
        items={article.evidenceBasis}
      />

      <section className="hs-section-white">
        <div className="hs-container hs-section">
          <Reveal className="max-w-3xl">
            <p className="hs-eyebrow">Sources</p>
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              Public records used for this guide
            </h2>
          </Reveal>
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {article.sourceNotes.map((source, index) => (
              <Reveal
                className="hs-card bg-[var(--hs-bg-soft)] p-5"
                key={source.href}
                staggerIndex={index}
              >
                <a
                  className="hs-text-link inline-flex items-start gap-2 font-extrabold"
                  href={source.href}
                  rel="noreferrer"
                  target="_blank"
                >
                  <span>{source.label}</span>
                  <ExternalLink aria-hidden className="mt-0.5 size-4 shrink-0" />
                </a>
                <p className="hs-muted mt-3 text-sm leading-6">{source.note}</p>
              </Reveal>
            ))}
          </div>
        </div>
      </section>

      <section className="hs-section-soft scroll-mt-24" id="documents">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            <div className="hs-icon-box size-12">
              <FileCheck2 aria-hidden className="size-6" />
            </div>
            <p className="hs-eyebrow mt-5">Inspection input</p>
            <h2 className="mt-2 text-3xl font-extrabold text-[var(--hs-text)]">
              What to send before the check
            </h2>
          </Reveal>
          <Reveal className="lg:col-span-8">
            <BulletList items={article.whatToSend} />
          </Reveal>
        </div>
      </section>

      <section className="hs-section-white">
        <div className="hs-container hs-section grid gap-8 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            <div className="hs-icon-box size-12">
              <AlertTriangle aria-hidden className="size-6" />
            </div>
            <p className="hs-eyebrow mt-5">Hold signals</p>
            <h2 className="mt-2 text-3xl font-extrabold text-[var(--hs-text)]">
              Lab report red flags
            </h2>
          </Reveal>
          <Reveal className="lg:col-span-8">
            <BulletList items={article.redFlags} />
          </Reveal>
        </div>
      </section>

      <section className="hs-section-soft scroll-mt-24" id="scope-limits">
        <div className="hs-container hs-section">
          <Reveal className="max-w-3xl">
            <p className="hs-eyebrow">Scope limits</p>
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              What this check cannot prove
            </h2>
            <p className="hs-muted mt-4 text-base leading-7">
              The evidence chain supports a better release decision; it does not turn a
              sourcing inspection into a laboratory, regulator, certifier, customs
              broker, or legal review.
            </p>
            <BulletList items={article.scopeLimits} />
          </Reveal>
        </div>
      </section>

      <section className="hs-section-white">
        <div className="hs-container hs-section">
          <Reveal className="max-w-3xl">
            <p className="hs-eyebrow">Related decisions</p>
            <h2 className="mt-3 text-3xl font-extrabold text-[var(--hs-text)]">
              Continue the evidence chain
            </h2>
          </Reveal>
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {article.relatedLinks.map((link, index) => (
              <Reveal key={link.href} staggerIndex={index}>
                <Link className="hs-card block h-full bg-[var(--hs-bg-soft)] p-5" href={link.href}>
                  <span className="inline-flex items-center gap-2 font-extrabold text-[var(--hs-accent-strong)]">
                    {link.label}
                    <ArrowRight aria-hidden className="size-4" />
                  </span>
                  <span className="hs-muted mt-3 block text-sm leading-6">{link.note}</span>
                </Link>
              </Reveal>
            ))}
          </div>
        </div>
      </section>

      <section className="hs-section-soft">
        <div className="hs-container hs-section">
          <Reveal className="rounded-[var(--hs-radius)] bg-[var(--hs-navy)] p-7 text-white shadow-[var(--hs-shadow-lg)] sm:p-9">
            <div className="grid items-center gap-6 lg:grid-cols-[1fr_auto]">
              <div>
                <div className="flex items-center gap-3">
                  <ShieldCheck aria-hidden className="size-7 text-[var(--hs-accent)]" />
                  <p className="text-sm font-extrabold uppercase tracking-wide text-[var(--hs-accent)]">
                    Before payment or pickup
                  </p>
                </div>
                <h2 className="mt-3 text-3xl font-extrabold">
                  Connect the report to the goods while they are still in China.
                </h2>
                <p className="mt-3 max-w-3xl text-sm leading-7 text-white/75">
                  Send the full reports, product file, lab details, factory and lot
                  records, change history, and release deadline. Huang Sourcing can
                  document China-side identity and production evidence for the
                  responsible buyer&apos;s qualified decision.
                </p>
              </div>
              <ContactAgentButton
                analyticsLabel={article.primaryCta.label}
                analyticsLocation="verify_china_lab_report_article_footer"
                className="hs-btn-primary h-12 px-6 text-sm"
                href={whatsappHref}
                size="lg"
                variant="default"
              >
                {article.primaryCta.label}
              </ContactAgentButton>
            </div>
          </Reveal>
        </div>
      </section>

      <SiteFooter />
    </main>
  )
}
